...
def multivar_normal_pdf(x, mu1, mu2, mu3, std1, std2, std3):
    ...
    """
    # NOTE ignoring prefactor since this a likelihood
...
os.makedirs('./data/kpca', exist_ok=True)
...
np.save('./data/kpca/xc.npy', XC)
XSTD = XALL.std(axis=1, keepdims=True)
np.save('./data/kpca/xstd.npy', XSTD)
XALL0 = (XALL - XC) / XSTD
...
def fit_sample():
    ...
    pca.fit(XALL0.T)
    PARAMS0 = pca.components_.flatten()
    ...
    for i, muv in tqdm(enumerate(muv_centers)):
        ...
        def p_x(x, y, f, fe, _muv, T, mode):
            ...
            # NOTE without the PCA basis, nothing forces Lha and Lya to be positively correlated
            # learning a new basis in each MUV bin loses cross-bin information

            plt.scatter(lly, lha, color='yellow', alpha=0.3, s=2)
            plt.scatter(lly[_p_obs>0.5], lha[_p_obs>0.5], color='orange', alpha=0.3, s=2)
            plt.scatter(x[0], x[2], color='red', alpha=0.3, s=2)
            plt.errorbar(mean_lya, mean_ha, xerr=std_lya, yerr=std_ha, \
                         color='white', marker='x', markersize=25)
            plt.text(np.sum(_p_obs)/len(_p_obs), 0.1, f'{mode.capitalize()} Sample', color='white', fontsize=20)
            plt.show()
            ...
            print(-1*np.mean(np.log10(_p_y)), -1*np.mean(np.log10(_p_f)))
            return np.log10(_p_y) + np.log10(_p_f)
        
        def loss(params):
            params = 0.5*(params + PARAMS0)
            T = params.reshape((3, 3))
            det = np.linalg.det(T)
            if det == 0:
                return 1e10 # Penalize singular matrices
            Y_MUV_WIDE = T @ ((X_MUV_WIDE - XC)/XSTD)
            Y_MUV_DEEP = T @ ((X_MUV_DEEP - XC)/XSTD)
            _p_x_wide = p_x(X_MUV_WIDE, Y_MUV_WIDE, f[i,0], f_err[i,0], _muv_wide, T, mode='wide')
            _p_x_deep = p_x(X_MUV_DEEP, Y_MUV_DEEP, f[i,1], f_err[i,1], _muv_deep, T, mode='deep')
            return -1*np.sum(_p_x_wide) -1*np.sum(_p_x_deep)

        bounds = [(-2, 2), (-2, 2), (-2, 2), (-2, 2), (-2, 2), \
                  (-2, 2), (-2, 2), (-2, 2), (-2, 2)]
        res = differential_evolution(loss, bounds, maxiter=300, disp=True)
        PARAMS0 = res.x
        all_params[i, :] = PARAMS0.copy()

    return muv_centers, all_params, all_n_points, f, f_err

muv_centers, all_params, all_n_points, f, f_err = fit_sample()

np.save('./data/kpca/muv.npy', muv_centers)
np.save('./data/kpca/params.npy', all_params)
np.save('./data/kpca/n_points.npy', all_n_points)
np.save('./data/kpca/f.npy', f)
np.save('./data/kpca/f_err.npy', f_err)
