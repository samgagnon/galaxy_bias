...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', type=int, default=100, help='Random seed')
    args = parser.parse_args()

    rs = args.r

    # Load the PTH file
    # 10.0.h5  8.176091259055681.h5  9.0.h5
    # pth = p21c.PerturbHaloField.from_file(get_PTH_fn(rs))
    pth = p21c.HaloField.from_file('data/halo_fields/HaloField_1027bf2c08c90b15f647a5dd1807d50c_r1.h5')
    ...
    np.save(f'./hmf_log10m.npy', bin_centers)
    np.save(f'./hmf_dndm.npy', hmf)
    np.save(f'./hmf_dndm_err.npy', log_hmf_asymmetric_error)
    ...
    for mode in modes:
        ...
        np.save(f'./{mode}_uv_phi.npy', uv_phi)
        np.save(f'./{mode}_uv_phi_err.npy', log_uv_asymmetric_error)
        np.save(f'./{mode}_uv_bin_centers.npy', bin_centers)
        ...
        np.save(f'./{mode}_lya_bin_centers.npy', bin_centers)
        ...
        np.save(f'./{mode}_W_bin_centers.npy', bin_centers)
        W_bins += [bin_centers]
        ...
        np.save(f'./{mode}_Lya_phi.npy', Lya_phi)
        np.save(f'./{mode}_Lya_phi_err.npy', log_Lya_asymmetric_error)
        np.save(f'./{mode}_W_density.npy', W_density)
        np.save(f'./{mode}_W_density_err.npy', W_density_err)