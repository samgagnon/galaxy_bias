...
def p_obs(lly, lha, muv, mode='wide'):
    ...
    # add factor for MUV
    _p_muv = p_muv(muv, phi_5, muv_star_5, alpha_5)
    
    return p_lya * p_ha * p_w * _p_muv
...
# Y = PCA T of X
os.makedirs('./data/nopca', exist_ok=True)
...
# axs[0].scatter(XALL[0], XALL[1], s=1, color='yellow', alpha=0.5)
# axs[0].set_xlabel(r'$L_{\rm Ly\alpha}$', fontsize=font_size)
# axs[0].set_ylabel(r'$\Delta_v$', fontsize=font_size)

# axs[1].scatter(XALL[0], XALL[2], s=1, color='yellow', alpha=0.5)
# axs[1].set_xlabel(r'$L_{\rm Ly\alpha}$', fontsize=font_size)
# axs[1].set_ylabel(r'$L_{\rm H\alpha}$', fontsize=font_size)

# axs[2].scatter(XALL[1], XALL[2], s=1, color='yellow', alpha=0.5)
# axs[2].set_xlabel(r'$\Delta_v$', fontsize=font_size)
# axs[2].set_ylabel(r'$L_{\rm H\alpha}$', fontsize=font_size)
...
np.save('./data/nopca/xc.npy', XC)
XSTD = XALL.std(axis=1, keepdims=True)
np.save('./data/nopca/xstd.npy', XSTD)
XALL0 = (XALL - XC) / XSTD
...
def fit_sample():
    ...
    all_params = np.zeros((20, 9))
    all_params_var = np.zeros((20, 9))
    ...
    for i, muv in tqdm(enumerate(muv_centers)):
        ...
        Y_MUV_WIDE = XWIDE0[:, select_wide]
        Y_MUV_DEEP = XDEEP0[:, select_deep]

        n_points = X_MUV_WIDE.shape[1]
        all_n_points[i] = n_points

        def p_x(x, y, f, fe, _muv, params, mode):
            mu1, mu2, mu3, std1, std2, std3, c1, c2, c3 = params
        ...
        mu1, mu2, mu3, std1, std2, std3, c1, c2, c3 = res.x
...
np.save('./data/nopca/muv.npy', muv_centers)
np.save('./data/nopca/params.npy', all_params)
np.save('./data/nopca/n_points.npy', all_n_points)
np.save('./data/nopca/f.npy', f)
np.save('./data/nopca/f_err.npy', f_err)
