...
def fit_sample():
    ...
    all_params = np.zeros((NBINS, 6))
    all_n_points = np.zeros((NBINS, 2))
    muv_centers = np.linspace(-20, -17, NBINS)
    f = np.zeros((NBINS, 2))
    f_err = np.zeros((NBINS, 2))
    # last_bounds = [(-1, 1), (-1, 1), (-1, 1), (0.01, 1), (0.01, 1), (0.01, 1)]
    last_bounds = [(-20, 5), (-20, 5), (-20, 5)]
    for i, muv in tqdm(enumerate(muv_centers)):
        ...
        def p_x(x, y, f, fe, _muv, params, mode):
            ...
            y1 = np.random.normal(mu1, 1, 1000)
            y2 = np.random.normal(mu2, 1, 1000)
            y3 = np.random.normal(mu3, 1, 1000)
            Y = np.stack([y1, y2, y3])
        ...
        all_params[i, :] = np.array([res.x[0], res.x[1], res.x[2], 1, 1, 1])
        # last_bounds = np.array([(p-0.1*np.sign(p)*p, p+0.1*np.sign(p)*p) for p in res.x])
