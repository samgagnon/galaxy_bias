...
XWIDEC = XWIDE.mean(axis=1, keepdims=True)
XDEEPC = XDEEP.mean(axis=1, keepdims=True)
XWIDESTD = XWIDE.std(axis=1, keepdims=True)
XDEEPSTD = XDEEP.std(axis=1, keepdims=True)
...
def fit_transform():
    ...
    popt1, pcov1 = curve_fit(line, XALL0[0],  XALL0[1], p0=[0])
    # fit straight line to Lya vs H-alpha luminosity
    popt2, pcov2 = curve_fit(line, XALL0[0],  XALL0[2], p0=[0])
    # fit straight line to Deltav vs H-alpha luminosity
    popt3, pcov3 = curve_fit(line, XALL0[1], XALL0[2], p0=[0])
    ...
    def objective_function(params):
        ...
        # muv_wide = np.random.choice(muv_space, size=1000, p=p_muv_space)
        muv_deep = np.random.choice(muv_space, size=1000, p=p_muv_space)
        # _p_obs_wide = p_obs(10**lly, 10**lha, muv_wide, mode='wide')
        _p_obs_deep = p_obs(10**lly, 10**lha, muv_deep, mode='deep')
        ...
        # logp_rho = -0.5*((popt2s-popt2)/err2)**2
        # print(f'logp_rho: {logp_rho.squeeze()}, params: {params}')
        # quit()
        # print(-1*logp_rho, params)

        # fig, axs = plt.subplots(1, 3, figsize=(12, 6), constrained_layout=True)
        # axs[0].scatter(XALL0[0], XALL0[1], color='red', s=5, alpha=0.2, label='PCA basis')
        # axs[0].scatter(lly, dv, c=_p_obs_deep, s=5, label='wide')
        # axs[0].plot(lly_deep, popt1s*lly_deep, color='lime', label='fit to deep sample')
        # axs[0].plot(XALL0[0], popt1*XALL0[0], color='green', label='fit to PCA basis')

        # axs[1].scatter(XALL0[0], XALL0[2], color='red', s=5, alpha=0.2, label='PCA basis')
        # axs[1].scatter(lly, lha, c=_p_obs_deep, s=5, label='deep')
        # axs[1].plot(lly_deep, popt2s*lly_deep, color='lime', label='fit to deep sample')
        # axs[1].plot(XALL0[0], popt2*XALL0[0], color='green', label='fit to PCA basis')

        # axs[2].scatter(XALL0[1], XALL0[2], color='red', s=5, alpha=0.2, label='PCA basis')
        # axs[2].scatter(dv, lha, c=_p_obs_deep, s=5, label='deep')
        # axs[2].plot(dv_deep, popt3s*dv_deep, color='lime', label='fit to deep sample')
        # axs[2].plot(XALL0[1], popt3*XALL0[1], color='green', label='fit to PCA basis')
        # plt.show()
...
# pca.fit(XALL0.T)
# A = pca.components_.T  # get the transformation matrix
...
_p_obs_wide = p_obs(10**lly, 10**lha, _muv_wide, mode='wide')
_p_obs_deep = p_obs(10**lly, 10**lha, _muv_deep, mode='deep')
