...
if __name__ == "__main__":
    ...
    STM_ref_dndm = np.load(f'.{dat_dir}/hmf_dndm_Jenkins.npy')*Planck18.h**4
    STM_ref_m = np.load(f'.{dat_dir}/hmf_m_SMT.npy')/Planck18.h
    ax_hmf.plot(np.log10(STM_ref_m), np.log10(STM_ref_dndm), color='white', linestyle='-', label='Jenkins')

    PS_ref_dndm = np.load(f'.{dat_dir}/hmf_dndm_PS.npy')*Planck18.h**4
    ...
    ax_hmf.errorbar(hmf_m, np.log10(hmf_dndm)-hmf_m, yerr=hmf_dndm_err, fmt='o', color='cyan', alpha=1.0, label=r'$21cmFASTv4$')
    ...
    for uv_phi, log_uv_asymmetric_error, bin_centers, mode, color in zip(uv_list, uv_err_list, uv_bins, mode_names, colors):
        ...
        axs[1].errorbar(bin_centers, np.log10(uv_phi), yerr=-1*log_uv_asymmetric_error*np.log10(uv_phi), \
                    fmt='o', color=color, alpha=1.0, label=mode)
    ...
    axs[2].errorbar(lum_silver, logphi_silver, yerr=asymmetric_error, fmt='o', color='white', label='Umeda+24')
    ...
    if z in [4.9, 5.7, 6.6]:
        ...
        # axs[3].fill_between(wrange_low, exp1_lower, exp1_upper, color='white', alpha=1.0)
        axs[3].plot(wrange_low, exp1, color='white', linestyle='--', label='Umeda+24')
        # axs[3].fill_between(wrange_high, exp2_lower, exp2_upper, color='white', alpha=1.0)
        axs[3].plot(wrange_high, exp2, color='white', linestyle='--')
